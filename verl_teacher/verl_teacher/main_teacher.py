...
def main(config):
    """Main entry point for training and scoring with the teacher model.

    Args:
        config_dict: Hydra configuration dictionary containing parameters.
    """
    run_ppo(config)
...
def run_ppo(config) -> None:
    """Initialize Ray cluster and run distributed process.

    Args:
        config: Configuration object containing all necessary parameters including Ray initialization settings,
...
class TaskRunner:
    ...
    def run(self, config):
        ...
        ray_worker_group_cls = self.add_teacher_workers(config)
...
# NOTE: Currently create_rl_dataset are create_rl_sampler identical to the verl-tool implementations
