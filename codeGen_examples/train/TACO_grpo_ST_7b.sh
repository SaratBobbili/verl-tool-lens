...
export DEBUGPY_LOG_DIR=/home/debugpy_logs
export DEBUGPY_LOG_LEVEL=debug

# Remove old records from test case execution
rm -rf ./verl_step_records/acecoder_records/

set -x
dataset_name=TACO-verified-filtered-single-turn
train_data=[$(pwd)/data/${dataset_name}/train.parquet]
val_data=[$(pwd)/data/${dataset_name}/test.parquet]
# train_data=/home/codeGen_curriculum/data/acecoder/AceCoderV2-122K-system-prompt-1/train.parquet
# val_data=/home/codeGen_curriculum/data/acecoder/AceCoderV2-122K-system-prompt-1/test.parquet

val_before_train=True
loggers="['console','wandb']" # Remove ,'wandb' while debugging
max_num_seqs=256
...
run_name="${reward_manager}-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}-${run_name_postfix}"
...
PYTHONUNBUFFERED=1 python3 -m verl_tool.trainer.main_ppo \
    ...
    trainer.project_name=codeGen_curriculum_new \
